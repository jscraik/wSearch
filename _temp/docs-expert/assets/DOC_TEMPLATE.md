# <Doc title as an informative sentence>

One sentence: what this doc helps the reader do, and who it is for.

Last updated: YYYY-MM-DD
Owner: <name/team>
Review cadence: <e.g., quarterly>

## Doc requirements
- Audience tier: <beginner/intermediate/expert>
- Scope: <what this doc covers>
- Non-scope: <what this doc does not cover>
- Required approvals: <names/roles>

## Table of contents
- [Doc requirements](#doc-requirements)
- [Risks and assumptions](#risks-and-assumptions)
- [Prerequisites](#prerequisites)
- [Quickstart](#quickstart)
- [Common tasks](#common-tasks)
- [Troubleshooting](#troubleshooting)
- [Acceptance criteria](#acceptance-criteria)
- [Evidence bundle](#evidence-bundle)
- [Reference](#reference)

## Risks and assumptions
- Assumptions: <what must be true>
- Risks / blast radius: <what can go wrong>
- Rollback / recovery: <how to recover>

## Prerequisites
- Required: <tool/version>, <access>, <accounts>
- Optional: <tooling>

## Quickstart
### 1) Do the minimum setup
```sh
# commands the repo actually supports
```

### 2) Run it
```sh
```

### 3) Verify
Expected output:
- <what success looks like>

## Common tasks
### Do <task> to achieve <result>
- What you get:
- Steps:
```sh
```
- Verify:

### Configure <thing> so that <result>
- Options table (if applicable):

| Option | Default | When to change | Example |
|---|---:|---|---|

## Troubleshooting
### Symptom: <what the reader sees>
Cause:
Fix:
```sh
```

(Repeat for top 3 issues.)

## Acceptance criteria
- [ ] <criterion 1>
- [ ] <criterion 2>
- [ ] <criterion 3>

## Evidence bundle
- Lint outputs (Vale/markdownlint/link check):
- Brand check output (if applicable):
- Readability output (if available):
- Checklist snapshot:

## Reference
- Key terms:
- Links to related docs:
- Constraints / limits:
